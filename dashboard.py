...
class DashboardUI:
    ...
    def render_sidebar(self):
        ...
        try:
            # Get all API keys
            api_keys = self.portfolio_manager.db.get_all_api_keys()
            
            if api_keys:
                # Get current active key
                active_key_id = st.session_state.get('active_api_key_id')
                if not active_key_id:
                    # If no active key in session, find the first active one
                    active_key = next((k for k in api_keys if k['is_active']), None)
                    active_key_id = active_key['id'] if active_key else None
                
                # Create a mapping of key names to IDs
                key_options = {f"{k['name']} ({'Active' if k['is_active'] else 'Inactive'})": k['id'] for k in api_keys}
                
                # Display the dropdown
                selected_key_name = st.sidebar.selectbox(
                    "Select API Key",
                    options=list(key_options.keys()),
                    index=list(key_options.values()).index(active_key_id) if active_key_id in key_options.values() else 0,
                    key="api_key_selector"
                )
                
                # Update session state if selection changed
                selected_key_id = key_options[selected_key_name]
                if selected_key_id != active_key_id:
                    st.session_state.active_api_key_id = selected_key_id
                    
                    # Update the active key in the database
                    for key in api_keys:
                        self.portfolio_manager.db.update_api_key(
                            key_id=key['id'],
                            is_active=(key['id'] == selected_key_id)
                        )
                    st.experimental_rerun()
                
                # Show status
                selected_key = next((k for k in api_keys if k['id'] == selected_key_id), None)
                if selected_key:
                    status = "✅" if selected_key['is_active'] else "⚠️"
                    st.sidebar.info(f"{status} {selected_key['name']} (ID: {selected_key['id']})")
            else:
                st.sidebar.warning("No API keys configured")
    ...
    def _render_manage_portfolio_form(self):
        ...
        # Create two columns for the header and delete button
        ...
        with col2:
            delete_clicked = st.button(
                "🗑️ Delete Portfolio", 
                type="secondary", 
                key=f"delete_portfolio_btn_{selected_portfolio.id}"
            )
            
            # Show confirmation dialog if delete was clicked
            if delete_clicked:
                st.session_state.confirm_delete = selected_portfolio.id
                st.rerun()
            
            if st.session_state.get('confirm_delete') == selected_portfolio.id:
                ...
                with col_confirm:
                    if st.button(
                        "Yes, delete it", 
                        key=f"confirm_delete_btn_{selected_portfolio.id}",
                        type="primary"
                    ):
                        # Check if portfolio has linked API keys
                        api_keys = self.portfolio_manager.db.get_api_keys_by_portfolio(selected_portfolio.id)
                        if api_keys:
                            st.error("Cannot delete portfolio with linked API keys. Please unlink or delete the API keys first.")
                            st.session_state.confirm_delete = None
                            st.rerun()
                        else:
                            if self.portfolio_manager.db.delete_portfolio(selected_portfolio.id):
                                st.success("Portfolio deleted successfully!")
                                # Clear any related session state
                                for key in ['confirm_delete', 'selected_portfolio']:
                ...
                with col_cancel:
                    if st.button("Cancel", key=f"cancel_delete_btn_{selected_portfolio.id}"):
